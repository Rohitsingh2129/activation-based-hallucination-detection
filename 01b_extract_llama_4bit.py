"""
Step 1b: Same as 01_extract_llama.py, but loads Llama-3.2-3B-Instruct
...
comparison point for Llama-3.2-3B — same model, same dataset, same
probing method as script 1, with only the precision changed. That
controlled comparison (fp16 vs 4-bit, everything else identical) is
exactly what your quantization-gap claim needs as evidence.

Output: data/activations_llama_truthfulqa_4bit.npz (same format as
script 1's output, so 02_train_probes.py works on this file unchanged —
just point DATA_PATH at it).
"""
...
def main():
    ...
    out_path = os.path.join(OUTPUT_DIR, "activations_llama_truthfulqa_4bit.npz")
    np.savez_compressed(
        out_path,
