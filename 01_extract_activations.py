"""
Step 1: Extract per-layer hidden states from Qwen-2.5-3B-Instruct on
...
- We extract ALL layers in one pass (not just one) because re-running the
  3B model forward pass is the expensive part; storing a few hundred MB of
...
def main():
    ...
    out_path = os.path.join(OUTPUT_DIR, "activations_truthfulqa.npz")
    np.savez_compressed(
        out_path,
