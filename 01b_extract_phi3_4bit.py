"""
Step 1b: Same as 01_extract_phi3.py, but loads Phi-3-mini-4k-instruct
...
comparison point for Phi-3-mini — same model, same dataset, same probing
...
Output: data/activations_phi3_truthfulqa_4bit.npz (same format as script
1's output, so 02_train_probes.py works on this file unchanged — just
...
def main():
    ...
    out_path = os.path.join(OUTPUT_DIR, "activations_phi3_truthfulqa_4bit.npz")
    np.savez_compressed(
        out_path,
